...
test_that("Binary search algorithm breaks if target is NA", {
  expect_error(binary_search(1:10, NA))
})

test_that("Binary search algorithm breaks if target is TRUE or FALSE", {
  expect_error(binary_search(0:10, TRUE))
  expect_error(binary_search(0:10, FALSE))
})
